...
#ifndef YGG_COMMON_BIT_PACKED_ARRAY_SET_HPP_
#define YGG_COMMON_BIT_PACKED_ARRAY_SET_HPP_
...
template <std::unsigned_integral Block,
          typename Coder = bit::ForwardingBlockCoder<Block>,
  ...
  void ensure_fits(std::span<const value_type> element) const {
    if (element.size() != length())
      throw std::invalid_argument(
          "BitPackedArraySet: wrong number of elements.");
  }

  index_type
  insert_new_with_matching_hash(size_t h, std::span<const value_type> element) {
    const auto index = to_uint_t(m_pool->size());
    m_pool->push_back(element);

    [[maybe_unused]] const auto [it, inserted] =
        m_set.emplace_with_hash(h, index);
    assert(inserted);
...
public:
  ...
  static size_t hash(std::span<const value_type> element) noexcept {
    return gtl::phmap_mix<sizeof(size_t)>()(Hash{}(element));
  }

  std::optional<index_type> find_with_hash(std::span<const value_type> element,
                                           size_t h) const {
    ensure_fits(element);
    assert(h == BitPackedArraySet::hash(element) &&
  ...
  bool contains_with_hash(std::span<const value_type> element, size_t h) const {
    return find_with_hash(element, h).has_value();
  }

  std::pair<index_type, bool>
  insert_with_hash(size_t h, std::span<const value_type> element) {
    ensure_fits(element);
    assert(h == BitPackedArraySet::hash(element) &&
           "The given hash does not match container internal's hash.");
    assert(h == m_set.hash(element));
  ...
  index_type insert_new_with_hash(size_t h,
                                  std::span<const value_type> element) {
    ensure_fits(element);
    assert(h == BitPackedArraySet::hash(element) &&
           "The given hash does not match container internal's hash.");
    assert(h == m_set.hash(element));
  ...
  ConstArrayView operator[](index_type index) const {
    return std::as_const(*m_pool)[index];
  }

  ConstArrayView at(index_type index) const {
    return std::as_const(*m_pool).at(index);
  }
...
private:
  void ensure_not_empty() const {
    if (empty())
      throw std::out_of_range("BitPackedArraySet: container is empty.");
  }
  ...
  public:
    ...
    explicit IndexableHash(const pool_type &pool_) noexcept
        : pool(&pool_), m_hash() {}

    size_t operator()(index_type idx) const noexcept {
      return m_hash((*pool)[idx]);
    }
    size_t operator()(std::span<const value_type> values) const noexcept {
  ...
  public:
    ...
    explicit IndexableEqualTo(const pool_type &pool_) noexcept
        : pool(&pool_), m_equal_to() {}

    bool operator()(index_type lhs, index_type rhs) const noexcept {
      return m_equal_to((*pool)[lhs], (*pool)[rhs]);
    }
    bool operator()(std::span<const value_type> lhs,
                    index_type rhs) const noexcept {
      return m_equal_to(lhs, (*pool)[rhs]);
    }
    bool operator()(index_type lhs,
                    std::span<const value_type> rhs) const noexcept {
