...
#ifndef YGG_COMMON_DYNAMIC_BITSET_HPP_
#define YGG_COMMON_DYNAMIC_BITSET_HPP_
...
template <std::unsigned_integral Block> class BitsetSpan {
private:
  template <std::unsigned_integral> friend class BitsetSpan;
...
public:
  ...
  static constexpr size_t block_index(size_t pos) noexcept {
    return pos >> BlockShift;
  }
  static constexpr size_t block_pos(size_t pos) noexcept {
    return pos & BlockMask;
  }
  static constexpr size_t num_blocks(size_t num_bits) noexcept {
    return (num_bits + Digits - 1) >> BlockShift;
  }
...
public:
  BitsetSpan(Block *data, size_t num_bits) noexcept
  ...
  std::span<const U> blocks() const noexcept {
    return {m_data, num_blocks(m_num_bits)};
  }
  ...
  void check_same_size(const BitsetSpan<OtherBlock> &other) const {
    if (m_num_bits != other.m_num_bits)
      throw std::invalid_argument(
          "BitsetSpan operands must have the same size.");
  }
...
constexpr bool operator==(const BitsetSpan<B1> &lhs,
                          const BitsetSpan<B2> &rhs) noexcept {
  ...
  const size_t n =
      BitsetSpan<std::remove_const_t<B1>>::num_blocks(lhs.num_bits());
...
template <typename Callback, typename BlockCombiner,
          std::unsigned_integral Block0,
          UnsignedIntegralSameAsIgnoringConst<Block0>... Blocks>
void for_each_bit(Callback &&callback, BlockCombiner &&combiner,
                  const BitsetSpan<Block0> &first,
                  const BitsetSpan<Blocks> &...rest) {
  ...
  assert(std::apply([&](auto const &...b) { return ((b.size() == n) && ...); },
                    rb));
  ...
  for (size_t block = 0; block < n; ++block) {
    ...
    U w = std::apply(
        [&](auto const &...b) noexcept -> U {
          return static_cast<U>(combiner(fb[block], b[block]...));
        },
        rb);
