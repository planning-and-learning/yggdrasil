...
public:
  RawVectorSet()
      : m_pool(std::make_shared<RawVectorPool<Size, T, FirstSegmentBytes>>()),
  ...
  RawVectorView<Size, T> operator[](uint_t idx) noexcept {
    return (*m_pool)[idx];
  }

  RawVectorView<const Size, const T> operator[](uint_t idx) const noexcept {
    return std::as_const(*m_pool)[idx];
  }

  RawVectorView<Size, T> at(uint_t idx) { return m_pool->at(idx); }

  RawVectorView<const Size, const T> at(uint_t idx) const {
    return std::as_const(*m_pool).at(idx);
  }

  RawVectorView<Size, T> front() { return m_pool->front(); }

  RawVectorView<const Size, const T> front() const {
    return std::as_const(*m_pool).front();
  }

  RawVectorView<Size, T> back() { return m_pool->back(); }

  RawVectorView<const Size, const T> back() const {
    return std::as_const(*m_pool).back();
  }
...
private:
  struct IndexableHash {
    ...
    explicit IndexableHash(
        std::shared_ptr<RawVectorPool<Size, T, FirstSegmentBytes>>
            pool) noexcept
        : pool(std::move(pool)) {}
  ...
  struct IndexableEqualTo {
    ...
    bool operator()(uint_t lhs, uint_t rhs) const noexcept {
      return equal_range((*pool)[lhs], (*pool)[rhs]);
    }
    ...
    bool operator()(uint_t lhs, std::span<const T> rhs) const noexcept {
      return equal_range((*pool)[lhs], rhs);
    }
