...
namespace ygg {
template <typename T, size_t FirstSegmentSize = 32> class SegmentedVector {
...
private:
  ...
  static size_t get_segment_index(size_t index) noexcept {
    return std::bit_width((index >> seg_shift) + size_t{1}) - 1;
  }
...
public:
  ...
  template <typename... Args> T &emplace_back(Args &&...args) {
    ...
    auto *element = std::construct_at(m_segments[index].data + offset,
