...
template <typename T, bool ThreadSafe = false> class SharedObjectPool;

template <typename T, bool ThreadSafe = false> struct SharedObjectPoolEntry;

template <typename T> struct SharedObjectPoolEntry<T, true> {
  ...
  explicit SharedObjectPoolEntry(Args &&...args)
      : refcount(0), object(std::forward<Args>(args)...) {}
...
template <typename T> struct SharedObjectPoolEntry<T, false> {
  size_t refcount;
  T object;
...
template <typename T, bool ThreadSafe = false> class SharedObjectPoolPtr;

template <typename T> class SharedObjectPoolPtr<T, true> {
private:
...
public:
  ...
  SharedObjectPoolPtr(SharedObjectPool<T, true> *pool, Entry *object) noexcept
      : m_pool(pool), m_entry(object) {
    if (m_pool && m_entry)
      inc_ref_count();
  }

  SharedObjectPoolPtr(const SharedObjectPoolPtr &other) noexcept
      : SharedObjectPoolPtr() {
    m_pool = other.m_pool;
...
template <typename T> class SharedObjectPoolPtr<T, false> {
private:
...
public:
  ...
  SharedObjectPoolPtr(SharedObjectPool<T, false> *pool, Entry *object) noexcept
      : m_pool(pool), m_entry(object) {
    if (m_pool && m_entry)
      inc_ref_count();
  }
...
template <typename T> class SharedObjectPool<T, true> {
private:
  using Entry = SharedObjectPoolEntry<T, true>;
...
template <typename T> class SharedObjectPool<T, false> {
private:
  using Entry = SharedObjectPoolEntry<T, false>;
