...
public:
  ...
  View(const Variant &handle, const C &context) noexcept
  ...
  template <typename U> decltype(auto) get() const noexcept {
  ...
  template <typename F> decltype(auto) apply(F &&f) const noexcept {
    return std::visit(
...
constexpr auto visit(Visitor &&vis,
                     View<::cista::offset::variant<T...>, C> &&v) noexcept {
...
constexpr auto
visit(Visitor &&vis,
      const View<::cista::offset::variant<T...>, C> &v) noexcept {
  return v.apply(vis);
}
} // namespace ygg
#endif
