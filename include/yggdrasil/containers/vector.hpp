...
template <typename T, template <typename> typename Ptr, bool IndexPointers,
          typename TemplateSizeType, class Allocator, typename C>
class View<
...
public:
  ...
  View(const Container &handle, const C &context) noexcept
  ...
  const_iterator begin() const noexcept {
    return const_iterator{get_data().data(), get_context()};
  }

  const_iterator end() const noexcept {
    return const_iterator{get_data().data() + get_data().size(), get_context()};
  }
...
template <class T>
void set(size_t pos, const T &value, std::vector<T> &vec,
...
template <typename T, size_t Rank> class MDSpan {
  static_assert(Rank > 0);

private:
  template <size_t K>
  static constexpr std::array<size_t, Rank - K>
  tail_k(const std::array<size_t, Rank> &a) noexcept {
  ...
  MDSpan(T *data, const std::array<size_t, Rank> &shapes,
         const std::array<size_t, Rank> &strides) noexcept
      : m_data(data), m_shapes(shapes), m_strides(strides) {}

public:
  MDSpan() : m_data(nullptr), m_shapes(), m_strides() {}
  MDSpan(T *data, const std::array<size_t, Rank> &shapes)
      : m_data(data), m_shapes(shapes), m_strides() {
    ...
    requires(Rank > 1)
  {
    ...
    return MDSpan<T, Rank - 1>(m_data + pos * m_strides[0], tail_k<1>(m_shapes),
                               tail_k<1>(m_strides));
  }
  ...
  {
    assert(pos < m_shapes[0]);
    return MDSpan<const T, Rank - 1>(m_data + pos * m_strides[0],
                                     tail_k<1>(m_shapes), tail_k<1>(m_strides));
  }
  ...
  auto operator()(Idx... idx) noexcept {
    ...
    return MDSpan<T, Rank - K>(m_data + off, tail_k<K>(m_shapes),
                               tail_k<K>(m_strides));
  }
  ...
  auto operator()(Idx... idx) const noexcept {
    constexpr size_t K = sizeof...(Idx);
    const size_t off = offset_prefix(idx...);
    return MDSpan<const T, Rank - K>(m_data + off, tail_k<K>(m_shapes),
                                     tail_k<K>(m_strides));
  }
...
private:
  ...
  template <typename... Idx> void ensure_indices_in_bounds(Idx... idx) const {
    size_t d = 0;
    bool in_bounds = true;
  ...
  template <typename... Idx> size_t offset_checked(Idx... idx) const {
  ...
  template <typename... Idx> size_t offset_prefix(Idx... idx) const noexcept {
    return offset(idx...);
  }

  template <typename... Idx> size_t offset_prefix_checked(Idx... idx) const {
