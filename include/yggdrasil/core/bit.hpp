...
template <std::unsigned_integral T> constexpr bool is_power_of_two(T x) {
  return std::has_single_bit(x);
}

template <std::unsigned_integral T> constexpr T ceil_div(T x, T y) {
  assert(y > 0);
  return x == 0 ? 0 : T(1) + (x - 1) / y;
}

template <std::unsigned_integral T> constexpr T bits_needed(T domain_size) {
  if (domain_size <= 1)
    return 1;
...
inline constexpr std::size_t bits_per_block_v =
 ...
 */
template <std::unsigned_integral Block> consteval auto make_lo_set() {
...
template <std::unsigned_integral Block>
constexpr void write_int(Block *word, Block x, uint8_t offset,
                         const uint8_t len) {
  constexpr std::size_t digits = std::numeric_limits<Block>::digits;
                              ...
                              // else case by exactly digits.
  {
    ...
    *word |= (x << offset);
    if ((offset =
             (offset + len) & (digits - 1))) // Number of bits written into
...
constexpr Block read_int(const Block *word, uint8_t offset, const uint8_t len) {
  ...
  if (offset + len > digits)
    return w1 | ((*(word + 1) & lo_set<Block>[(offset + len) & (digits - 1)])
                 << (digits - offset));
  else
...
concept BlockCoder = std::unsigned_integral<Block> &&
                     requires(Block block, typename Encoder::value_type value) {
                       {
                         Encoder::decode(block)
                       } -> std::convertible_to<typename Encoder::value_type>;
                       { Encoder::encode(value) } -> std::convertible_to<Block>;
                     };

template <std::unsigned_integral Block> struct ForwardingBlockCoder {
...
template <std::unsigned_integral Block,
          BlockCoder<Block> Coder = bit::ForwardingBlockCoder<Block>>
class int_reference {
private:
...
public:
  ...
  constexpr int_reference(Block *word, uint8_t offset, uint8_t len) noexcept
  ...
  constexpr int_reference &operator=(const int_reference &other) {
    return (*this = static_cast<value_type>(other));
  }

  constexpr operator value_type() const {
    return Coder::decode(bit::read_int(m_word, m_offset, m_len));
  }
...
public:
  ...
  constexpr block_reference &operator=(const block_reference &other) {
    return (*this = static_cast<value_type>(other));
  }
...
template <std::unsigned_integral Block> struct bit_reference {
  Block *data;
  size_t bit;
  ...
  explicit operator bool() const noexcept {
    return ((data[block_index(bit)] >> bit_index(bit)) & Block(1)) != 0;
  }
};
