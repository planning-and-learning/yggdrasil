...
template <typename T> struct is_cista_variant_helper : std::false_type {};

template <typename... Ts>
struct is_cista_variant_helper<::cista::offset::variant<Ts...>>
    : std::true_type {};

template <typename T>
concept CistaVariantConcept = is_cista_variant_helper<T>::value;

template <typename T> struct is_optional_variant_helper : std::false_type {};

template <typename T>
struct is_optional_variant_helper<::cista::optional<T>> : std::true_type {};

template <typename T>
concept CistaOptionalConcept =
    is_optional_variant_helper<std::remove_cvref_t<T>>::value;

template <typename T> void clear(T &element) {
  if constexpr (Clearable<T>)
    element.clear();
  else if constexpr (CistaVariantConcept<T>) {
    static_assert(std::is_default_constructible_v<T>,
                  "Cannot clear variant: not default constructible.");
...
template <std::ranges::input_range Range, typename List>
  requires requires(std::ranges::range_reference_t<Range> view,
                    List &ref_list) { append(view, ref_list); }
...
template <typename T, typename C>
void set(const std::optional<View<Index<T>, C>> &view,
         ::cista::optional<Index<T>> &data) {
  if (view) {
    data = view->get_index();
  } else {
    data = std::nullopt;
...
template <typename T, typename C>
void set(const std::optional<View<Data<T>, C>> &view,
         ::cista::optional<Data<T>> &data) {
  if (view) {
    data = view->get_data();
  } else {
    data = std::nullopt;
...
void set(const Range &views, List &out_list) {
  ...
  if constexpr (std::ranges::sized_range<Range> &&
                requires(List &list, const Range &range) {
                  list.reserve(std::ranges::size(range));
                }) {
