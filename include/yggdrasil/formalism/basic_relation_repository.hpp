...
template <typename ObjectTag, typename T> class BasicRelationRepository {
private:
  template <std::unsigned_integral Block> struct Coder {
    using value_type = Index<Object<ObjectTag>>;

    static constexpr value_type decode(Block block) noexcept {
      return value_type(block);
    }
    static constexpr Block encode(value_type value) noexcept {
      return static_cast<Block>((static_cast<ygg::uint_t>(value)));
    }
  };

  static constexpr ygg::uint_t kInvalid =
      std::numeric_limits<ygg::uint_t>::max();
  ...
  struct Slot {
    ...
    Slot(Index<T> g, size_t arity, size_t parent_size)
        : g(g), container(arity), parent_size(parent_size) {}
  };
...
public:
  ...
  BasicRelationRepository(const BasicRelationRepository *parent = nullptr)
      : m_parent(parent), m_forward(), m_slots() {
    clear_slots();
  }
  BasicRelationRepository(const BasicRelationRepository &other) = delete;
  BasicRelationRepository &
  operator=(const BasicRelationRepository &other) = delete;
  ...
  hash(const Data<RelationBinding<T, ObjectTag>> &builder) noexcept {
    return ygg::BlockArraySet<ygg::uint_t, Coder<ygg::uint_t>>::hash(
        builder.objects);
  }

  std::optional<Index<Row>>
  find_local_with_hash(const Data<RelationBinding<T, ObjectTag>> &builder,
    ...
    if (auto row_or_nullopt =
            slot->container.find_with_hash(builder.objects, h))
      return Index<Row>(slot->parent_size + *row_or_nullopt);

    return std::nullopt;
  }

  std::optional<Index<Row>> find_local(
      const Data<RelationBinding<T, ObjectTag>> &builder) const noexcept {
    return find_local_with_hash(builder,
                                BasicRelationRepository::hash(builder));
  }

  std::pair<Index<Row>, bool> get_or_create_local_with_hash(
  ...
  std::pair<Index<Row>, bool>
  get_or_create_local(const Data<RelationBinding<T, ObjectTag>> &builder) {
    return get_or_create_local_with_hash(
        builder, BasicRelationRepository::hash(builder));
  }
  ...
  ConstViewType at_local(Index<RelationBinding<T, ObjectTag>> index) const {
    ...
    const auto *slot = find_slot(g);
    if (!slot || row.value < slot->parent_size ||
        row.value >= slot->parent_size + slot->container.size())
      throw std::out_of_range(
          "Relation binding index not found in local repository.");
  ...
  bool is_local(Index<RelationBinding<T, ObjectTag>> index) const noexcept {
    ...
    return ygg::uint_t(row) >= slot->parent_size &&
           ygg::uint_t(row) < slot->parent_size + slot->container.size();
  }
