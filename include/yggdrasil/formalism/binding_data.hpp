...
struct Data<ygg::formalism::RelationBinding<Relation, ObjectTag>> {
  ...
  Data() = default;
  Data(Index<Relation> relation_, size_t arity,
       IndexList<ygg::formalism::Object<ObjectTag>> objects_)
      : relation(relation_), objects(std::move(objects_)) {
    if (objects.size() != arity)
      throw std::invalid_argument(
          "RelationBinding object count does not match relation arity.");
  }
  // Python constructor
  template <typename C>
  Data(::ygg::View<Index<Relation>, C> relation_,
       const std::vector<
           ::ygg::View<Index<ygg::formalism::Object<ObjectTag>>, C>> &objects_)
      : relation(), objects() {
  ...
  auto identifying_members() const noexcept {
    return std::tie(relation, objects);
  }
};
