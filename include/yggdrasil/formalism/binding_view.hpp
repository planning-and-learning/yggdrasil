...
public:
  View(Index<ygg::formalism::RelationBinding<Relation, ObjectTag>> handle,
       const C &context) noexcept
  ...
  auto get_relation() const noexcept {
    return ygg::make_view(m_handle.relation, *m_context);
  }
  auto get_objects() const noexcept {
    return ygg::make_view(get_data(), *m_context);
  }
  auto get_key() const noexcept {
    return std::make_pair(get_relation(), get_objects());
  }

  auto identifying_members() const noexcept {
    return std::tie(m_handle, m_context->get_index());
  }
};

template <typename Relation, typename ObjectTag,
          std::ranges::forward_range BindingRange, typename C>
  requires std::same_as<
      std::remove_cvref_t<std::ranges::range_reference_t<BindingRange>>,
      Index<ygg::formalism::Row>>
class View<ygg::formalism::RelationBindingsForwardRange<Relation, ObjectTag,
                                                        BindingRange>,
           C> {
public:
  using Container =
      ygg::formalism::RelationBindingsForwardRange<Relation, ObjectTag,
                                                   BindingRange>;
  using T = Index<ygg::formalism::RelationBinding<Relation, ObjectTag>>;
  using I1 = Index<Relation>;

  View(Container handle, const C &context) noexcept
      : m_context(&context), m_handle(handle) {}

  bool empty() const noexcept {
    return std::ranges::begin(get_data().rows) ==
           std::ranges::end(get_data().rows);
  }
  ...
  struct const_iterator {
    ...
    using value_type =
        std::conditional_t<ViewConcept<T, C>, ::ygg::View<T, C>, T>;
    using iterator_category = std::forward_iterator_tag;
    using iterator_concept = std::forward_iterator_tag;

    const_iterator() noexcept : ctx(nullptr), it() {}
    const_iterator(I1 relation, BaseIt it, const C &ctx) noexcept
        : ctx(&ctx), it(it), relation(relation) {}
  ...
  const_iterator begin() const noexcept {
    return const_iterator{get_data().relation,
                          std::ranges::begin(get_data().rows), get_context()};
  }

  const_iterator end() const noexcept {
    return const_iterator{get_data().relation,
                          std::ranges::end(get_data().rows), get_context()};
  }
...
template <typename Relation, typename ObjectTag,
          std::ranges::random_access_range BindingRange, typename C>
  requires std::ranges::sized_range<BindingRange> &&
           std::same_as<std::remove_cvref_t<
                            std::ranges::range_reference_t<BindingRange>>,
                        Index<ygg::formalism::Row>>
class View<ygg::formalism::RelationBindingsRandomAccessRange<
               Relation, ObjectTag, BindingRange>,
           C> {
public:
  using Container =
      ygg::formalism::RelationBindingsRandomAccessRange<Relation, ObjectTag,
                                                        BindingRange>;
  using T = Index<ygg::formalism::RelationBinding<Relation, ObjectTag>>;
  using I1 = Index<Relation>;
  ...
  decltype(auto) back() const {
    ensure_not_empty();
    auto it = std::ranges::begin(get_data().rows) +
              (std::ranges::ssize(get_data().rows) - 1);
    if constexpr (ViewConcept<T, C>)
      return ygg::make_view(T{get_data().relation, *it}, get_context());
    else
      return T{get_data().relation, *it};
  ...
  decltype(auto) operator[](size_t i) const noexcept {
    auto it =
        std::ranges::begin(get_data().rows) + static_cast<std::ptrdiff_t>(i);
    if constexpr (ViewConcept<T, C>)
      return ygg::make_view(T{get_data().relation, *it}, get_context());
    else
      return T{get_data().relation, *it};
  ...
  void ensure_not_empty() const {
    if (empty())
      throw std::out_of_range(
          "RelationBindingsRandomAccessRange: range is empty.");
  }
