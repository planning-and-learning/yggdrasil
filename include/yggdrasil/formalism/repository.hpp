...
private:
    ...
    template<typename T>
    std::optional<View<Index<RelationBinding<T, typename RelationRepo::object_tag>>, Repository>> find_with_hash(const Data<RelationBinding<T, typename RelationRepo::object_tag>>& builder, size_t h) const noexcept
    {
        ...
        while (current != nullptr)
        {
            if (auto row_or_nullopt = current->m_relation_repository.find_local_with_hash(builder, h))
                return View<Index<RelationBinding<T, typename RelationRepo::object_tag>>, Repository>(Index<RelationBinding<T, typename RelationRepo::object_tag>> { g, *row_or_nullopt }, *current);
    ...
    template<typename T>
    std::pair<View<Index<RelationBinding<T, typename RelationRepo::object_tag>>, Repository>, bool> get_or_create_with_hash(const Data<RelationBinding<T, typename RelationRepo::object_tag>>& builder, size_t h)
    {
        const auto g = builder.relation;
        ...
        {
            if (auto row_or_nullopt = current->m_relation_repository.find_local_with_hash(builder, h))
                return { View<Index<RelationBinding<T, typename RelationRepo::object_tag>>, Repository>(Index<RelationBinding<T, typename RelationRepo::object_tag>> { g, *row_or_nullopt }, *current), false };
            current = current->m_parent;
        }
        ...
        return { View<Index<RelationBinding<T, typename RelationRepo::object_tag>>, Repository>(Index<RelationBinding<T, typename RelationRepo::object_tag>> { g, row }, *this), success };
    }
...
public:
    ...
    template<typename T>
    std::optional<View<Index<RelationBinding<T, typename RelationRepo::object_tag>>, Repository>> find(const Data<RelationBinding<T, typename RelationRepo::object_tag>>& builder) const noexcept
    {
        return find_with_hash(builder, RelationRepo::hash(builder));
    }

    template<typename T>
    std::pair<View<Index<RelationBinding<T, typename RelationRepo::object_tag>>, Repository>, bool> get_or_create(const Data<RelationBinding<T, typename RelationRepo::object_tag>>& builder)
    {
        return get_or_create_with_hash(builder, RelationRepo::hash(builder));
    }

    template<typename T>
    auto operator[](Index<RelationBinding<T, typename RelationRepo::object_tag>> index) const noexcept
    {
        const Repository* current = this;
        while (current != nullptr)
        {
    ...
    template<typename T>
    const Repository& get_canonical_context(Index<RelationBinding<T, typename RelationRepo::object_tag>> index) const noexcept
    {
        const Repository* current = this;
        while (current != nullptr)
        {
