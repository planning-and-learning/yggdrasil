...
template <typename SymbolRepo, typename RelationRepo> class Repository {
private:
  ...
  std::optional<View<Index<T>, Repository>>
  find_with_hash(const Data<T> &builder, size_t h) const noexcept {
    const Repository *current = this;
    while (current != nullptr) {
      if (auto index_or_nullopt =
              current->m_symbol_repository.find_local_with_hash(builder, h))
  ...
  template <typename T>
    requires NonRelationBindingConcept<T>
  std::pair<View<Index<T>, Repository>, bool>
  get_or_create_with_hash(Data<T> &builder, size_t h) {
    const Repository *current = this;
    while (current != nullptr) {
      if (auto index_or_nullopt =
              current->m_symbol_repository.find_local_with_hash(builder, h))
    ...
    const auto [index, success] =
        m_symbol_repository.get_or_create_local_with_hash(builder, h);
  ...
  template <typename T>
  std::optional<View<
      Index<RelationBinding<T, typename RelationRepo::object_tag>>, Repository>>
  find_with_hash(
      const Data<RelationBinding<T, typename RelationRepo::object_tag>>
          &builder,
    ...
    while (current != nullptr) {
      if (auto row_or_nullopt =
              current->m_relation_repository.find_local_with_hash(builder, h))
        return View<
            Index<RelationBinding<T, typename RelationRepo::object_tag>>,
            Repository>(
            Index<RelationBinding<T, typename RelationRepo::object_tag>>{
                g, *row_or_nullopt},
            *current);
  ...
  template <typename T>
  std::pair<View<Index<RelationBinding<T, typename RelationRepo::object_tag>>,
                 Repository>,
            bool>
  get_or_create_with_hash(
      const Data<RelationBinding<T, typename RelationRepo::object_tag>>
          &builder,
      size_t h) {
    const auto g = builder.relation;
    ...
    while (current != nullptr) {
      if (auto row_or_nullopt =
              current->m_relation_repository.find_local_with_hash(builder, h))
        return {
            View<Index<RelationBinding<T, typename RelationRepo::object_tag>>,
                 Repository>(
                Index<RelationBinding<T, typename RelationRepo::object_tag>>{
                    g, *row_or_nullopt},
                *current),
    ...
    const auto [row, success] =
        m_relation_repository.get_or_create_local_with_hash(builder, h);
    return {View<Index<RelationBinding<T, typename RelationRepo::object_tag>>,
                 Repository>(
                Index<RelationBinding<T, typename RelationRepo::object_tag>>{
                    g, row},
                *this),
...
public:
  Repository(size_t index, const Repository *parent = nullptr)
      : m_parent(parent), m_root(m_parent ? m_parent->m_root : this),
        m_symbol_repository(m_parent ? &m_parent->m_symbol_repository
                                     : nullptr),
        m_relation_repository(m_parent ? &m_parent->m_relation_repository
                                       : nullptr),
        m_index(index) {
    clear();
  }
  ...
  template <typename T>
    requires NonRelationBindingConcept<T>
  std::optional<View<Index<T>, Repository>>
  find(const Data<T> &builder) const noexcept {
  ...
  std::optional<View<
      Index<RelationBinding<T, typename RelationRepo::object_tag>>, Repository>>
  find(const Data<RelationBinding<T, typename RelationRepo::object_tag>>
           &builder) const noexcept {
  ...
  template <typename T>
  auto
  operator[](Index<RelationBinding<T, typename RelationRepo::object_tag>> index)
      const {
    const Repository *current = this;
    while (current != nullptr) {
  ...
  template <typename T> auto front(Index<T> g) const {
    const Repository *current = this;
    while (current->m_parent != nullptr) {
  ...
  template <typename T> size_t size(Index<T> g) const noexcept {
    return m_relation_repository.parent_size(g) +
           m_relation_repository.local_size(g);
  }

  template <typename T>
  const Repository &get_canonical_context(
      Index<RelationBinding<T, typename RelationRepo::object_tag>> index)
      const {
    const Repository *current = this;
    while (current != nullptr) {
      if (current->m_relation_repository.is_local(index))
