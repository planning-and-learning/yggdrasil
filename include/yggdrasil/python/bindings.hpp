...
template <typename V> nb::class_<V> &add_print(nb::class_<V> &cls) {
  return cls //
      .def("__str__", [](const V &self) { return to_string(self); })
      .def("__repr__", [](const V &self) { return to_string(self); });
}

template <typename V> nb::class_<V> &add_hash(nb::class_<V> &cls) {
  return cls //
      .def("__eq__", [](const V &self,
                        const V &other) { return EqualTo<V>{}(self, other); })
      .def("__hash__", [](const V &self) { return Hash<V>{}(self); });
}

template <typename T> void bind_index(nb::module_ &m, const std::string &name) {
  nb::class_<T>(m, name.c_str()) //
      ...
      .def("__repr__", [name](const T &i) {
        return name + "(" + std::to_string(static_cast<uint_t>(i)) + ")";
      });
}
...
void bind_fixed_uint(nb::module_ &m, const std::string &name) {
  ...
  nb::class_<T>(m, name.c_str())
      ...
      .def(
          "__add__", [](const T &a, value_type b) { return a + b; },
          nb::is_operator())
      .def(
          "__sub__", [](const T &a, value_type b) { return a - b; },
          nb::is_operator())
      .def(
          "__iadd__",
          [](T &a, value_type b) {
      ...
      .def("__repr__", [name](const T &x) {
        return name + "(" + std::to_string(static_cast<value_type>(x)) + ")";
      });
}
