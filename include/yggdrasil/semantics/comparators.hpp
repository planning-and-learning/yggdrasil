...
#ifndef YGG_COMMON_COMPARATORS_HPP_
#define YGG_COMMON_COMPARATORS_HPP_
...
template <typename... Ts> struct Less<std::tuple<Ts...>> {
...
private:
  template <size_t I>
  static bool less_impl(const std::tuple<Ts...> &lhs,
                        const std::tuple<Ts...> &rhs) noexcept {
...
template <typename T, size_t N> struct Less<std::array<T, N>> {
...
template <typename T> struct Less<std::reference_wrapper<T>> {
...
template <typename T> struct Less<std::optional<T>> {
  bool operator()(const std::optional<T> &lhs,
    ...
    return lhs.has_value()
               ? Less<std::remove_cvref_t<T>>{}(lhs.value(), rhs.value())
               : false;
  }
};

template <typename... Ts> struct Less<std::variant<Ts...>> {
...
template <typename T, std::size_t Extent> struct Less<std::span<T, Extent>> {
...
template <Identifiable T> struct Less<T> {
  using is_transparent = void;

  using MembersTupleType = decltype(std::declval<T>().identifying_members());

  bool operator()(const T &lhs, const T &rhs) const noexcept {
    return Less<std::remove_cvref_t<MembersTupleType>>{}(
...
template <Identifiable T> struct LessEqual<T> {
  using is_transparent = void;

  using MembersTupleType = decltype(std::declval<T>().identifying_members());

  bool operator()(const T &lhs, const T &rhs) const noexcept {
    return LessEqual<std::remove_cvref_t<MembersTupleType>>{}(
        lhs.identifying_members(), rhs.identifying_members());
  }
...
template <Identifiable T> struct Greater<T> {
  using is_transparent = void;

  using MembersTupleType = decltype(std::declval<T>().identifying_members());

  bool operator()(const T &lhs, const T &rhs) const noexcept {
    return Greater<std::remove_cvref_t<MembersTupleType>>{}(
        lhs.identifying_members(), rhs.identifying_members());
  }
...
template <Identifiable T> struct GreaterEqual<T> {
  using is_transparent = void;

  using MembersTupleType = decltype(std::declval<T>().identifying_members());

  bool operator()(const T &lhs, const T &rhs) const noexcept {
    return GreaterEqual<std::remove_cvref_t<MembersTupleType>>{}(
        lhs.identifying_members(), rhs.identifying_members());
  }
