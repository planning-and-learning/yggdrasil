...
template <> struct EqualTo<::cista::offset::string> {
...
template <typename... Ts> struct EqualTo<::cista::offset::variant<Ts...>> {
...
template <typename T> struct EqualTo<::cista::optional<T>> {
...
struct EqualTo<View<::cista::optional<T>, C>> {
  ...
  bool operator()(const Type &lhs, const Type &rhs) const noexcept {
    ...
    return EqualTo<std::remove_cvref_t<decltype(lhs.value())>>{}(lhs.value(),
                                                                 rhs.value());
  }
};
