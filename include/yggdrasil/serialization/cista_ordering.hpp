...
template <typename Ptr> struct Less<::cista::basic_string<Ptr>> {
...
template <typename T> struct Less<::cista::optional<T>> {
...
template <typename C, typename T> struct Less<View<::cista::optional<T>, C>> {
  ...
  bool operator()(const Type &lhs, const Type &rhs) const noexcept {
    ...
    return lhs.has_value() ? Less<std::remove_cvref_t<decltype(lhs.value())>>{}(
                                 lhs.value(), rhs.value())
                           : false;
  }
};

template <typename... Ts> struct Less<::cista::offset::variant<Ts...>> {
  ...
  bool operator()(const Type &lhs, const Type &rhs) const noexcept {
    ...
    return lhs.apply([&](auto &&l) {
      return rhs.apply([&](auto &&r) {
