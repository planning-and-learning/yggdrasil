...
{
template<>
struct Index<tests::BufferIndexedHashSetTag> : IndexMixin<Index<tests::BufferIndexedHashSetTag>>
{
...
template<>
struct Data<tests::BufferIndexedHashSetTag>
{
...
inline bool is_canonical(const Data<tests::BufferIndexedHashSetTag>&) { return true; }
...
TEST(YggdrasilTests, BufferIndexedHashSetDefaultConstructionSupportsReadOnlyQueries)
{
    auto repository = buffer::IndexedHashSet<BufferIndexedHashSetTag>();
    const auto element = Data<BufferIndexedHashSetTag> { Index<BufferIndexedHashSetTag>(0), 10, 2 };
...
TEST(YggdrasilTests, BufferIndexedHashSetSupportsHashAwareInsertion)
{
    ...
    const auto first = Data<BufferIndexedHashSetTag> { Index<BufferIndexedHashSetTag>(0), 10, 2 };
    const auto second = Data<BufferIndexedHashSetTag> { Index<BufferIndexedHashSetTag>(1), 11, 3 };
    ...
    const auto [first_index, first_inserted] = repository.insert_with_hash(first_hash, first);
    ...
    const auto [duplicate_index, duplicate_inserted] = repository.insert_with_hash(first_hash, first);
    EXPECT_FALSE(duplicate_inserted);
    EXPECT_EQ(duplicate_index, first_index);

    EXPECT_EQ(repository.insert_new_with_hash(second_hash, second), Index<BufferIndexedHashSetTag>(1));
    EXPECT_EQ(repository.find_with_hash(second, second_hash), Index<BufferIndexedHashSetTag>(1));
}
...
TEST(YggdrasilTests, BufferIndexedHashSetStoresSerializedCanonicalData)
{
    auto arena = buffer::SegmentedBuffer();
    auto bytes = buffer::Buffer();
    auto repository = buffer::IndexedHashSet<BufferIndexedHashSetTag>(bytes, arena);
