...
TEST(YggdrasilTests, CommonBitPackedArrayViewAtChecksBounds) {
  ...
  const auto const_view =
      ygg::BasicBitPackedArrayView<const uint8_t,
                                   ygg::bit::ForwardingBlockCoder<uint8_t>>(
          storage.data(), 2, 3, 0);
  ...
  auto invalid_view = ygg::BasicBitPackedArrayView<
      uint8_t, ygg::bit::ForwardingBlockCoder<uint8_t>>(nullptr, 2, 3, 0);
  const auto invalid_const_view = ygg::BasicBitPackedArrayView<
      const uint8_t, ygg::bit::ForwardingBlockCoder<uint8_t>>(nullptr, 2, 3, 0);
...
TEST(YggdrasilTests,
     CommonBitPackedArrayViewFrontBackCheckEmptyAndInvalidViews) {
  auto storage = std::array<uint8_t, 1>{};
  auto view =
      ygg::BasicBitPackedArrayView<uint8_t,
                                   ygg::bit::ForwardingBlockCoder<uint8_t>>(
...
TEST(YggdrasilTests, CommonBitPackedArrayRejectsInvalidWidths) {
  ...
  EXPECT_THROW(
      ...
      std::invalid_argument);
  EXPECT_THROW(
      (ygg::BitPackedArrayPool<uint8_t, ygg::bit::ForwardingBlockCoder<uint8_t>,
                               1>(1, 0)),
      std::invalid_argument);
  EXPECT_THROW(
      (ygg::BitPackedArrayPool<uint8_t, ygg::bit::ForwardingBlockCoder<uint8_t>,
...
TEST(YggdrasilTests, CommonBitPackedArrayPoolOutOfRange) {
  auto pool = ygg::BitPackedArrayPool<
      ygg::uint_t, ygg::bit::ForwardingBlockCoder<ygg::uint_t>, 1>(2, 2);

  // 4 requires width 3, which exceeds the limit of 2.
  EXPECT_THROW(pool.push_back(std::vector<ygg::uint_t>({1, 4})),
               std::out_of_range);

  // 3 elements are too much for a pool that stores arrays of length 2.
  EXPECT_THROW(pool.push_back(std::vector<ygg::uint_t>({1, 1, 1})),
               std::invalid_argument);
}
