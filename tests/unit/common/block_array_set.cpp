...
TEST(YggdrasilTests, CommonBlockArrayViewReportsEmpty) {
  ...
  auto view = ygg::BasicBlockArrayView<uint8_t,
                                       ygg::bit::ForwardingBlockCoder<uint8_t>>(
      storage.data(), storage.size());
  auto empty_view = ygg::BasicBlockArrayView<
      uint8_t, ygg::bit::ForwardingBlockCoder<uint8_t>>(storage.data(), 0);
...
TEST(YggdrasilTests, CommonBlockArrayViewAtChecksBounds) {
  ...
  const auto const_view =
      ygg::BasicBlockArrayView<const uint8_t,
                               ygg::bit::ForwardingBlockCoder<uint8_t>>(
          storage.data(), storage.size());
  ...
  auto invalid_view =
      ygg::BasicBlockArrayView<uint8_t,
                               ygg::bit::ForwardingBlockCoder<uint8_t>>(nullptr,
                                                                        2);
  const auto invalid_const_view =
      ygg::BasicBlockArrayView<const uint8_t,
                               ygg::bit::ForwardingBlockCoder<uint8_t>>(nullptr,
                                                                        2);
  ...
  EXPECT_THROW((invalid_view = std::span<const uint8_t>(storage)),
               std::logic_error);
}
...
TEST(YggdrasilTests, CommonBlockArrayPoolReturnsInsertedIndex) {
  auto pool =
      ygg::BlockArrayPool<uint8_t, ygg::bit::ForwardingBlockCoder<uint8_t>, 1>(
          2);
...
TEST(YggdrasilTests, CommonBlockArraySetOutOfRange) {
  auto set =
      BlockArraySet<ygg::uint_t, bit::ForwardingBlockCoder<ygg::uint_t>, 1>(2);

  // 3 elements are too much for a pool that stores arrays of length 2.
  EXPECT_THROW(set.insert(std::vector<ygg::uint_t>({1, 1, 1})),
               std::invalid_argument);
  EXPECT_THROW(set.find(std::vector<ygg::uint_t>({1, 1, 1})),
               std::invalid_argument);
  EXPECT_THROW(set.contains(std::vector<ygg::uint_t>({1})),
               std::invalid_argument);
}

TEST(YggdrasilTests, CommonBlockArraySetEmptyAccessThrows) {
  auto set =
      BlockArraySet<uint8_t, ygg::bit::ForwardingBlockCoder<uint8_t>, 1>(2);
...
TEST(YggdrasilTests, CommonBlockArraySetDeduplicatesZeroLengthArrays) {
  auto set =
      BlockArraySet<uint8_t, ygg::bit::ForwardingBlockCoder<uint8_t>, 1>(0);
  const auto empty = std::vector<uint8_t>{};
...
TEST(YggdrasilTests, CommonBlockArraySetSupportsHashAwareInsertion) {
  using Set =
      ygg::BlockArraySet<uint8_t, ygg::bit::ForwardingBlockCoder<uint8_t>, 1>;
  auto set = Set(2);
  ...
  const auto [first_index, first_inserted] =
      set.insert_with_hash(first_hash, first);
  EXPECT_TRUE(first_inserted);
  EXPECT_EQ(first_index, 0);
  ...
  for (size_t i = 0; i < 5; ++i) {
    ...
    for (uint8_t a = 0; a < 4; ++a) {
      for (uint8_t b = 0; b < 4; ++b) {
        ...
        EXPECT_TRUE(set.contains_with_hash(
            value,
            BlockArraySet<uint8_t, ygg::bit::ForwardingBlockCoder<uint8_t>,
                          1>::hash(value)));
