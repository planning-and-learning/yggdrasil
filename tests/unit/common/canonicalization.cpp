...
struct CanonicalizationContext {
  ...
  const ygg::Data<CanonicalizationTestTag> &
  operator[](ygg::Index<CanonicalizationTestTag> index) const;
};
} // namespace ygg::tests

namespace ygg {
template <> struct Data<ygg::tests::CanonicalizationTestTag> {
  ygg::uint_t value{};
...
template <>
struct Index<ygg::tests::CanonicalizationTestTag>
    : IndexMixin<Index<ygg::tests::CanonicalizationTestTag>> {
...
namespace fmt {
template <>
struct formatter<ygg::View<ygg::Data<ygg::tests::CanonicalizationTestTag>,
                           ygg::tests::CanonicalizationContext>,
                 char> {
  constexpr auto parse(format_parse_context &ctx) { return ctx.begin(); }

  template <typename FormatContext>
  auto format(const ygg::View<ygg::Data<ygg::tests::CanonicalizationTestTag>,
                              ygg::tests::CanonicalizationContext> &value,
              FormatContext &ctx) const {
    return fmt::format_to(
        ctx.out(), "{}", value.get_context().labels.at(value.get_data().value));
  }
};
} // namespace fmt

namespace ygg::tests {
const ygg::Data<CanonicalizationTestTag> &CanonicalizationContext::operator[](
    ygg::Index<CanonicalizationTestTag> index) const {
...
TEST(
    YggdrasilTests,
    CommonCanonicalizeContextAwareIndexListSortsAndDeduplicatesByFormattedData) {
  const auto data =
      std::vector<ygg::Data<Tag>>{{.value = 2}, {.value = 0}, {.value = 0}};
  const auto context =
      CanonicalizationContext{.data = &data, .labels = {"a", "b", "c"}};
  auto list = ygg::IndexList<Tag>{};
...
TEST(
    YggdrasilTests,
    CommonCanonicalizeContextAwareDataListSortsAndDeduplicatesByFormattedData) {
  const auto context =
      CanonicalizationContext{.data = nullptr, .labels = {"a", "b", "c"}};
  auto list = ygg::DataList<Tag>{};
  list.push_back(ygg::Data<Tag>{.value = 2});
