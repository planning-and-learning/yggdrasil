...
concept OrderedByAllCommonPredicates =
    ygg::LessFor<ygg::Less<T>, T> && ygg::LessFor<ygg::LessEqual<T>, T> &&
    ygg::LessFor<ygg::Greater<T>, T> && ygg::LessFor<ygg::GreaterEqual<T>, T>;
...
TEST(YggdrasilTests, CommonOrderingPredicatesCoverHashAndEqualToFamilies) {
  ...
  using BitPackedView =
      ygg::BasicBitPackedArrayView<uint8_t,
                                   ygg::bit::ForwardingBlockCoder<uint8_t>>;
  ...
  static_assert(
      OrderedByAllCommonPredicates<ygg::View<CistaVector, ComparatorContext>>);
  static_assert(OrderedByAllCommonPredicates<CistaOptional>);
  static_assert(OrderedByAllCommonPredicates<
                ygg::View<CistaOptional, ComparatorContext>>);
  static_assert(OrderedByAllCommonPredicates<CistaVariant>);
  static_assert(
      OrderedByAllCommonPredicates<ygg::View<CistaVariant, ComparatorContext>>);
  ...
  static_assert(
      OrderedByAllCommonPredicates<ygg::View<BlockView, ComparatorContext>>);
  static_assert(OrderedByAllCommonPredicates<BitPackedView>);
  static_assert(OrderedByAllCommonPredicates<
                ygg::View<BitPackedView, ComparatorContext>>);
...
TEST(YggdrasilTests, CommonLessRangeMatchesContainerLess) {
  ...
  EXPECT_EQ(
      ygg::less_range(std::span<const int>(lhs), std::span<const int>(rhs)),
      ygg::Less<std::span<const int>>{}(std::span<const int>(lhs),
...
TEST(YggdrasilTests, CommonLessOrdersVariantAlternativesByIndex) {
  ...
  EXPECT_TRUE(
      ygg::Less<DuplicateTypeVariant>{}(duplicate_first, duplicate_second));
  EXPECT_FALSE(
      ygg::Less<DuplicateTypeVariant>{}(duplicate_second, duplicate_first));
}
...
TEST(YggdrasilTests, CommonDynamicBitsetComparatorOrdersBitsetSpans) {
  auto lhs_blocks =
      std::vector<uint64_t>(ygg::BitsetSpan<uint64_t>::num_blocks(8), 0);
  auto rhs_blocks =
      std::vector<uint64_t>(ygg::BitsetSpan<uint64_t>::num_blocks(8), 0);
...
TEST(YggdrasilTests, CommonCistaLessAdaptersOrderOffsetVectorViews) {
  ...
  EXPECT_TRUE(ygg::Less<VectorView>{}(VectorView(lhs, context),
                                      VectorView(rhs, context)));
  EXPECT_FALSE(ygg::Less<VectorView>{}(VectorView(rhs, context),
                                       VectorView(lhs, context)));
}
...
TEST(YggdrasilTests, CommonCistaLessAdaptersOrderOptionalViews) {
  ...
  EXPECT_TRUE(ygg::Less<OptionalView>{}(OptionalView(lhs, context),
                                        OptionalView(rhs, context)));
  EXPECT_FALSE(ygg::Less<OptionalView>{}(OptionalView(rhs, context),
                                         OptionalView(lhs, context)));
  EXPECT_TRUE(ygg::Less<OptionalView>{}(OptionalView(empty, context),
                                        OptionalView(lhs, context)));
  EXPECT_FALSE(ygg::Less<OptionalView>{}(OptionalView(lhs, context),
                                         OptionalView(empty, context)));
}
...
TEST(YggdrasilTests, CommonCistaLessAdaptersOrderVariantViews) {
  ...
  EXPECT_TRUE(ygg::Less<VariantView>{}(VariantView(lhs, context),
                                       VariantView(rhs, context)));
  EXPECT_FALSE(ygg::Less<VariantView>{}(VariantView(rhs, context),
                                        VariantView(lhs, context)));
  EXPECT_TRUE(ygg::Less<VariantView>{}(
      VariantView(lhs, context), VariantView(different_alternative, context)));
  EXPECT_FALSE(ygg::Less<VariantView>{}(
      VariantView(different_alternative, context), VariantView(lhs, context)));
  EXPECT_TRUE(ygg::Less<VariantView>{}(VariantView(invalid, context),
                                       VariantView(lhs, context)));
  EXPECT_FALSE(ygg::Less<VariantView>{}(VariantView(lhs, context),
                                        VariantView(invalid, context)));
  EXPECT_FALSE(ygg::Less<VariantView>{}(VariantView(invalid, context),
