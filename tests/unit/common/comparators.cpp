...
template<typename T>
concept OrderedByAllCommonPredicates = ygg::LessFor<ygg::Less<T>, T> && ygg::LessFor<ygg::LessEqual<T>, T> && ygg::LessFor<ygg::Greater<T>, T> && ygg::LessFor<ygg::GreaterEqual<T>, T>;
...
TEST(YggdrasilTests, CommonLessRangeMatchesContainerLess)
{
    ...
    EXPECT_EQ(ygg::less_range(std::span<const int>(lhs), std::span<const int>(rhs)), ygg::Less<std::span<const int>> {}(std::span<const int>(lhs), std::span<const int>(rhs)));
}
