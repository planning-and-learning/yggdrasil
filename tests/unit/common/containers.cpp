...
TEST(YggdrasilTests, CommonArrayViewsExposeHandlesAndRandomAccessIterators) {
  ...
  using BlockView =
      ygg::BasicBlockArrayView<unsigned,
                               ygg::bit::ForwardingBlockCoder<unsigned>>;
  ...
  static_assert(
      std::same_as<decltype(block_view.get_handle()), const BlockView &>);
  ...
  auto bit_data =
      ygg::BasicBitPackedArrayView<unsigned,
                                   ygg::bit::ForwardingBlockCoder<unsigned>>(
          bit_storage.data(), 2, 2, 0);
  ...
  static_assert(
      std::same_as<decltype(bit_view.get_handle()), const BitPackedView &>);
...
TEST(YggdrasilTests, CommonArrayViewFrontBackPropagateEmptyViewErrors) {
  ...
  const auto empty_block_view =
      ygg::View<BlockView, int>(empty_block_data, context);
  ...
  const auto empty_bit_view =
      ygg::View<BitPackedView, int>(empty_bit_data, context);
...
TEST(YggdrasilTests, CommonContainersUmbrellaHeaderCompiles) {
  ...
  int visited_value = 0;
  EXPECT_TRUE(ygg::visit_at(tuple, 1,
                            [&](const auto &value) { visited_value = value; }));
  EXPECT_EQ(visited_value, 2);

  EXPECT_FALSE(
      ygg::visit_at(tuple, 2, [&](const auto &) { visited_value = 9; }));
  EXPECT_EQ(visited_value, 2);

  const auto const_tuple = std::tuple<int, int>{3, 4};
  EXPECT_TRUE(ygg::visit_at(const_tuple, 0,
                            [&](const auto &value) { visited_value = value; }));
  ...
  auto optional_view =
      ygg::View<::cista::optional<int>, int>(optional, context);
  ...
  const auto invalid_variant_view =
      ygg::View<Variant, int>(invalid_variant, context);
