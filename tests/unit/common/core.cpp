...
namespace ygg {
template <> struct Data<tests::CoreTypeUtilsTag> : tests::CoreTypeUtilsData {
  using CoreTypeUtilsData::CoreTypeUtilsData;
};

template <>
struct Index<tests::CoreTypeUtilsTag>
    : IndexMixin<Index<tests::CoreTypeUtilsTag>> {
...
struct CoreTypeUtilsContext {
  ...
  const ygg::Data<CoreTypeUtilsTag> &
  operator[](ygg::Index<CoreTypeUtilsTag> index) const {
    return data.at(index.get_value());
  }
};

template <typename T> using CoreAddConst = const T;

template <typename Bound, typename T> using CorePair = std::pair<Bound, T>;
...
TEST(YggdrasilTests, CommonCoreUmbrellaExposesLightweightHelpers) {
  ...
  EXPECT_EQ(ygg::bit::ceil_div(std::numeric_limits<unsigned>::max(), 2u),
  ...
  auto packed_ref =
      ygg::bit::int_reference<uint8_t>(packed_blocks.data(), 0, 3);
  ...
  EXPECT_THROW(
      ygg::to_uint_t(
          static_cast<size_t>(std::numeric_limits<ygg::uint_t>::max()) + 1),
      std::overflow_error);
  ...
  *observer = 2;
  ...
  static_assert(
      std::is_same_v<decltype(const_observer), ygg::ObserverPtr<const int>>);
  EXPECT_EQ(*const_observer, 3);
  EXPECT_EQ(ygg::Hash<ygg::ObserverPtr<const int>>{}(const_observer),
            ygg::Hash<int>{}(3));
  EXPECT_TRUE(ygg::EqualTo<ygg::ObserverPtr<const int>>{}(
      const_observer, ygg::make_observer(const_value)));
  ...
  EXPECT_EQ(ygg::common::resolve_path(prefix, "/absolute"),
  ...
  static_assert(
      std::is_same_v<ApplyTypeListT<std::tuple, TypeList<int, double>>,
                     std::tuple<int, double>>);
  static_assert(
      std::is_same_v<MapTypeListT<CoreAddConst, TypeList<int, double>>,
                     TypeList<const int, const double>>);
  static_assert(
      std::is_same_v<MapTypeListSecondT<CorePair, int, TypeList<float, double>>,
                     TypeList<std::pair<int, float>, std::pair<int, double>>>);
  static_assert(
      std::is_same_v<ConcatTypeListsT<TypeList<int>, TypeList<float, double>,
                                      TypeList<char>>,
                     TypeList<int, float, double, char>>);

  static_assert(
      !std::is_copy_constructible_v<StopwatchScope<std::chrono::nanoseconds>>);
  static_assert(
      !std::is_move_constructible_v<StopwatchScope<std::chrono::nanoseconds>>);
}
...
TEST(YggdrasilTests, CommonCoreObserverPtrExposesNonOwningValueSemantics) {
  ...
  *first_observer = 3;
  ...
  first = 1;
  EXPECT_TRUE(ygg::EqualTo<ygg::ObserverPtr<int>>{}(first_observer,
                                                    also_first_observer));
  EXPECT_FALSE(
      ygg::EqualTo<ygg::ObserverPtr<int>>{}(first_observer, second_observer));
  EXPECT_TRUE(
      ygg::Less<ygg::ObserverPtr<int>>{}(first_observer, second_observer));
  EXPECT_EQ(ygg::Hash<ygg::ObserverPtr<int>>{}(first_observer),
            ygg::Hash<int>{}(first));
}

TEST(YggdrasilTests, CommonCoreTypeUtilsExtendsAnyInputRange) {
  auto context = CoreTypeUtilsContext{
      {ygg::Data<CoreTypeUtilsTag>{1}, ygg::Data<CoreTypeUtilsTag>{2}}};
  const auto first =
      ygg::View<ygg::Index<CoreTypeUtilsTag>, CoreTypeUtilsContext>(
          ygg::Index<CoreTypeUtilsTag>(0), context);
  const auto second =
      ygg::View<ygg::Index<CoreTypeUtilsTag>, CoreTypeUtilsContext>(
          ygg::Index<CoreTypeUtilsTag>(1), context);
...
TEST(YggdrasilTests, CommonCoreCartesianSetResetsWorkspaceOnEarlyReturns) {
  ...
  ygg::itertools::cartesian_set::for_each_element(
      first_ranges.begin(), first_ranges.end(), workspace,
  ...
  ygg::itertools::cartesian_set::for_each_element(
      range_with_empty_inner.begin(), range_with_empty_inner.end(), workspace,
      [&](const auto &) { FAIL(); });
  ...
  ygg::itertools::cartesian_set::for_each_element(
      empty_outer.begin(), empty_outer.end(), workspace,
      [&](const auto &element) {
