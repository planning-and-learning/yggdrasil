...
TEST(YggdrasilTests, CommonDynamicBitset) {
  auto lhs_blocks =
      std::vector<uint64_t>(ygg::BitsetSpan<uint64_t>::num_blocks(70), 0);
  auto rhs_blocks =
      std::vector<uint64_t>(ygg::BitsetSpan<uint64_t>::num_blocks(70), 0);
...
TEST(YggdrasilTests, CommonDynamicBitsetAtChecksBounds) {
  ...
  const auto const_bitset =
      ygg::BitsetSpan<const uint64_t>(blocks.data(), bitset.size());
...
TEST(YggdrasilTests,
     CommonDynamicBitsetFindNextHandlesSentinelAndEndPositions) {
  auto blocks =
      std::vector<uint64_t>(ygg::BitsetSpan<uint64_t>::num_blocks(70), 0);
  auto bitset = ygg::BitsetSpan<uint64_t>(blocks.data(), 70);

  EXPECT_EQ(bitset.find_first(), ygg::BitsetSpan<uint64_t>::npos);
  EXPECT_EQ(bitset.find_next(ygg::BitsetSpan<uint64_t>::npos),
            ygg::BitsetSpan<uint64_t>::npos);
  EXPECT_EQ(bitset.find_next_zero(ygg::BitsetSpan<uint64_t>::npos),
            ygg::BitsetSpan<uint64_t>::npos);
...
TEST(YggdrasilTests, CommonDynamicBitsetRejectsMismatchedSpanSizes) {
  auto lhs_blocks =
      std::vector<uint64_t>(ygg::BitsetSpan<uint64_t>::num_blocks(8), 0);
  auto rhs_blocks =
      std::vector<uint64_t>(ygg::BitsetSpan<uint64_t>::num_blocks(9), 0);
...
TEST(YggdrasilTests,
     CommonDynamicBitsetBoostHelpersTreatOutOfRangeTestAsFalseAndResizeOnSet) {
...
TEST(YggdrasilTests, CommonDynamicBitsetAdaptersHashAndCompareSpans) {
  ...
  EXPECT_EQ(ygg::Hash<ygg::BitsetSpan<uint64_t>>{}(lhs),
            ygg::Hash<ygg::BitsetSpan<uint64_t>>{}(rhs));

  rhs.set(2);

  EXPECT_FALSE(ygg::EqualTo<ygg::BitsetSpan<uint64_t>>{}(lhs, rhs));
  EXPECT_NE(ygg::Hash<ygg::BitsetSpan<uint64_t>>{}(lhs),
            ygg::Hash<ygg::BitsetSpan<uint64_t>>{}(rhs));
