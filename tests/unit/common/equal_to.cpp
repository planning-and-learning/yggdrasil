...
TEST(YggdrasilTests, CommonEqualRangeMatchesContainerEqualTo) {
  ...
  EXPECT_EQ(ygg::equal_range(lhs, rhs),
            ygg::EqualTo<std::vector<int>>{}(lhs, rhs));
  EXPECT_EQ(
      ygg::equal_range(std::span<const int>(lhs), std::span<const int>(rhs)),
      ygg::EqualTo<std::span<const int>>{}(std::span<const int>(lhs),
                                           std::span<const int>(rhs)));
}
...
TEST(YggdrasilTests, CommonEqualToAdaptersCompareVariantAlternativeIndex) {
  ...
  EXPECT_FALSE(ygg::EqualTo<DuplicateTypeVariant>{}(first_alternative,
                                                    second_alternative));
}

TEST(YggdrasilTests,
     CommonReferenceWrapperEqualToAdaptersCompareReferencedValues) {
  auto lhs_value = 7;
  auto rhs_value = 7;
...
TEST(YggdrasilTests,
     CommonCistaEqualToAdaptersCompareOffsetStringOptionalAndVariant) {
  ...
  EXPECT_FALSE(
      ygg::EqualTo<::cista::offset::string>{}(lhs_string, different_string));
  ...
  EXPECT_TRUE(
      ygg::EqualTo<::cista::optional<int>>{}(lhs_optional, rhs_optional));
  EXPECT_FALSE(
      ygg::EqualTo<::cista::optional<int>>{}(lhs_optional, different_optional));
  EXPECT_FALSE(
      ygg::EqualTo<::cista::optional<int>>{}(lhs_optional, empty_optional));
  ...
  EXPECT_FALSE(
      ygg::EqualTo<DuplicateTypeVariant>{}(duplicate_first, duplicate_second));
}
...
TEST(YggdrasilTests,
     CommonDynamicBitsetEqualToAdaptersCompareBoostDynamicBitsets) {
...
TEST(YggdrasilTests, CommonDynamicBitsetEqualToAdaptersCompareBitsetSpans) {
  ...
  EXPECT_FALSE(
      ygg::EqualTo<ygg::BitsetSpan<const std::uint64_t>>{}(lhs, different));
}
...
TEST(YggdrasilTests, CommonCistaEqualToAdaptersCompareViews) {
  ...
  EXPECT_TRUE(ygg::EqualTo<VectorView>{}(VectorView(lhs_vector, context),
                                         VectorView(rhs_vector, context)));
  EXPECT_FALSE(ygg::EqualTo<VectorView>{}(
      VectorView(lhs_vector, context), VectorView(different_vector, context)));
  ...
  EXPECT_TRUE(
      ygg::EqualTo<OptionalView>{}(OptionalView(lhs_optional, context),
                                   OptionalView(rhs_optional, context)));
  EXPECT_FALSE(
      ygg::EqualTo<OptionalView>{}(OptionalView(lhs_optional, context),
                                   OptionalView(different_optional, context)));
  ...
  EXPECT_TRUE(ygg::EqualTo<VariantView>{}(VariantView(lhs_variant, context),
                                          VariantView(rhs_variant, context)));
  EXPECT_FALSE(
      ygg::EqualTo<VariantView>{}(VariantView(lhs_variant, context),
                                  VariantView(different_variant, context)));
  EXPECT_TRUE(ygg::EqualTo<VariantView>{}(VariantView(Variant{}, context),
                                          VariantView(Variant{}, context)));
  EXPECT_FALSE(ygg::EqualTo<VariantView>{}(VariantView(Variant{}, context),
                                           VariantView(lhs_variant, context)));
}
...
TEST(YggdrasilTests, CommonBlockArrayEqualToAdaptersCompareViews) {
  ...
  EXPECT_TRUE(ygg::EqualTo<WrappedView>{}(WrappedView(lhs, context),
                                          WrappedView(rhs, context)));
  EXPECT_FALSE(ygg::EqualTo<WrappedView>{}(WrappedView(lhs, context),
                                           WrappedView(different, context)));
}
...
TEST(YggdrasilTests, CommonRawAndSegmentedVectorEqualToAdaptersCompareValues) {
  ...
  EXPECT_TRUE((ygg::EqualTo<ygg::RawVectorView<uint8_t, int>>{}(
      pool[lhs_index], pool[rhs_index])));
  EXPECT_FALSE((ygg::EqualTo<ygg::RawVectorView<uint8_t, int>>{}(
      pool[lhs_index], pool[different_index])));
