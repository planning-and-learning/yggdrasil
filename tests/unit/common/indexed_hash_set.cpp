...
struct IndexedHashSetCustomHash {
  std::size_t
  operator()(const ygg::Data<IndexedHashSetTestTag> &value) const noexcept;
};

struct IndexedHashSetCustomEqualTo {
  bool operator()(const ygg::Data<IndexedHashSetTestTag> &lhs,
...
template <> struct Data<ygg::tests::IndexedHashSetTestTag> {
  int value{};

  auto identifying_members() const noexcept { return std::tie(value); }
};

inline bool is_canonical(const Data<ygg::tests::IndexedHashSetTestTag> &) {
  return true;
}
...
template <>
struct Index<ygg::tests::IndexedHashSetTestTag>
    : IndexMixin<Index<ygg::tests::IndexedHashSetTestTag>> {
...
inline std::size_t IndexedHashSetCustomHash::operator()(
    const ygg::Data<IndexedHashSetTestTag> &value) const noexcept {
  return static_cast<std::size_t>(value.value);
}

inline bool IndexedHashSetCustomEqualTo::operator()(
    const ygg::Data<IndexedHashSetTestTag> &lhs,
    const ygg::Data<IndexedHashSetTestTag> &rhs) const noexcept {
...
static_assert(ygg::HashFor<ygg::tests::IndexedHashSetCustomHash,
                           ygg::Data<ygg::tests::IndexedHashSetTestTag>>);
static_assert(ygg::EqualToFor<ygg::tests::IndexedHashSetCustomEqualTo,
                              ygg::Data<ygg::tests::IndexedHashSetTestTag>>);
...
TEST(YggdrasilTests, CommonIndexedHashSetSupportsHashAwareInsertion) {
  using Set =
      ygg::IndexedHashSet<IndexedHashSetTestTag, IndexedHashSetCustomHash,
                          IndexedHashSetCustomEqualTo>;
  auto set = Set();
  ...
  EXPECT_EQ(set.insert_new_with_hash(second_hash, second),
            ygg::Index<IndexedHashSetTestTag>(1));
  EXPECT_EQ(set.find_with_hash(second, second_hash),
            ygg::Index<IndexedHashSetTestTag>(1));
}
