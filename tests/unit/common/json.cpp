...
TEST(YggdrasilTests, CommonJsonHeaderAccessesRequiredAndOptionalMembers) {
  ...
  EXPECT_EQ(common::find_string(object, "name", "root"),
            std::optional<std::string>("case"));
  EXPECT_EQ(common::find_size(object, "count", "root"),
            std::optional<size_t>(3));
  EXPECT_EQ(common::find_uint_t(object, "count", "root"),
            std::optional<ygg::uint_t>(3));
  EXPECT_EQ(common::find_bool(object, "enabled", "root"),
            std::optional<bool>(true));
  EXPECT_EQ(common::find_double(object, "ratio", "root"),
  ...
  if constexpr (std::numeric_limits<size_t>::max() >
    ...
    const auto unsigned_count = boost::json::value(
        static_cast<std::uint64_t>(std::numeric_limits<std::int64_t>::max()) +
        1);
    EXPECT_EQ(common::as_size(unsigned_count, "root.unsigned_count"),
              static_cast<size_t>(std::numeric_limits<std::int64_t>::max()) +
                  1);
  } else if constexpr (std::numeric_limits<size_t>::max() <
                       static_cast<std::uint64_t>(
                           std::numeric_limits<std::int64_t>::max())) {
    const auto too_large_for_size = boost::json::value(
        static_cast<std::int64_t>(std::numeric_limits<size_t>::max()) + 1);
    EXPECT_THROW(common::as_size(too_large_for_size, "root.count"),
                 std::runtime_error);
  }
...
TEST(YggdrasilTests, CommonJsonHeaderReportsTypedErrors) {
  const auto value = boost::json::parse(
      R"({"name":3,"count":-1,"enabled":1,"score":{},"items":1})");
  const auto &object = common::as_object(value, "root");
  ...
  expect_error([&] { return common::as_string(object, "name", "root"); },
               "root.name must be a string.");
  expect_error([&] { return common::as_size(object, "count", "root"); },
               "root.count must be a non-negative integer.");
  expect_error([&] { return common::as_uint_t(object, "count", "root"); },
               "root.count must be a non-negative integer.");
  expect_error([&] { return common::as_bool(object, "enabled", "root"); },
               "root.enabled must be a boolean.");
  expect_error([&] { return common::as_double(object, "score", "root"); },
               "root.score must be a number or \"NaN\".");
  expect_error([&] { return common::find_object(object, "items", "root"); },
               "root.items must be an object.");
  expect_error([&] { return common::as_array(object, "items", "root"); },
               "root.items must be an array.");
  expect_error([&] { return common::find_array(object, "name", "root"); },
               "root.name must be an array.");
  expect_error(
      [&] { return common::as_object(object.at("name"), "root.name"); },
      "root.name must be an object.");
  expect_error([&] { return common::as_array(object.at("name"), "root.name"); },
               "root.name must be an array.");
  const auto too_large_for_uint = boost::json::value(
      static_cast<std::uint64_t>(std::numeric_limits<ygg::uint_t>::max()) + 1);
  expect_error(
      [&] { return common::as_uint_t(too_large_for_uint, "root.count"); },
      "root.count must fit into uint_t.");
  expect_error(
      [&] { return common::require_member(object, "missing", "root"); },
      "root.missing is required.");
}
