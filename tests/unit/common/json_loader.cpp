...
TEST(YggdrasilTests, CommonJsonLoaderResolvePath) {
  ...
  EXPECT_EQ(common::resolve_path(prefix, "child/file.json"),
            prefix / "child/file.json");
  EXPECT_EQ(common::resolve_path(prefix, "/absolute/file.json"),
            std::filesystem::path("/absolute/file.json"));
}

TEST(YggdrasilTests, CommonJsonLoaderReadAndLoadFile) {
  const auto path = std::filesystem::temp_directory_path() /
                    "yggdrasil_json_loader_test.json";
  {
  ...
  EXPECT_TRUE(common::as_bool(common::as_object(object, "nested", "root"),
                              "enabled", "root.nested"));
  ...
  EXPECT_EQ(common::find_uint_t(object, "count", "root"),
            std::optional<uint_t>(3));
  EXPECT_EQ(common::find_bool(common::as_object(object, "nested", "root"),
                              "enabled", "root.nested"),
...
TEST(YggdrasilTests, CommonJsonLoaderReportsParseErrorsWithPath) {
  const auto path = std::filesystem::temp_directory_path() /
                    "yggdrasil_json_loader_invalid_test.json";
  {
    auto out = std::ofstream(path);
    out << R"({"name":)";
  }
...
TEST(YggdrasilTests, CommonJsonLoaderAccessesOptionalAndRequiredMembers) {
  ...
  EXPECT_EQ(common::as_string(common::require_member(object, "name", "root"),
                              "root.name"),
            "case");
  EXPECT_THROW(common::require_member(object, "missing", "root"),
               std::runtime_error);
}

TEST(YggdrasilTests, CommonJsonLoaderReportsTypedErrors) {
  const auto value = boost::json::parse(
      R"({"name":3,"count":-1,"enabled":1,"score":{},"items":1,"nested":[]})");
  const auto &object = common::as_object(value, "root");
  ...
  EXPECT_THROW(common::find_uint_t(object, "count", "root"),
               std::runtime_error);
  EXPECT_THROW(common::find_bool(object, "enabled", "root"),
               std::runtime_error);
  EXPECT_THROW(common::find_double(object, "score", "root"),
               std::runtime_error);
  EXPECT_THROW(common::find_array(object, "items", "root"), std::runtime_error);
  EXPECT_THROW(common::find_object(object, "nested", "root"),
               std::runtime_error);
}
