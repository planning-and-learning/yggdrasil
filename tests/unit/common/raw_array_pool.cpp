...
TEST(YggdrasilTests, CommonRawArrayPoolRejectsImpossibleSegmentSizes) {
  ...
  constexpr auto too_large =
      std::numeric_limits<size_t>::max() / arrays_per_segment + 1;

  EXPECT_THROW((ygg::RawArrayPool<int, arrays_per_segment>(too_large)),
               std::length_error);
}
...
TEST(YggdrasilTests, CommonRawArrayPoolStoresFixedLengthArrays) {
  ...
  EXPECT_EQ(std::vector<int>(pool[0], pool[0] + pool.array_size()),
            (std::vector<int>{1, 2, 3}));
  EXPECT_EQ(std::vector<int>(pool.front(), pool.front() + pool.array_size()),
            (std::vector<int>{1, 2, 3}));
  EXPECT_EQ(std::vector<int>(pool.back(), pool.back() + pool.array_size()),
            (std::vector<int>{4, 5, 6}));
  EXPECT_EQ(std::vector<int>(pool[1], pool[1] + pool.array_size()),
            (std::vector<int>{4, 5, 6}));
}
...
TEST(YggdrasilTests, CommonRawArrayPoolAtChecksBounds) {
  ...
  EXPECT_EQ(std::vector<int>(pool.at(0), pool.at(0) + pool.array_size()),
            (std::vector<int>{1, 5}));
  EXPECT_EQ(std::vector<int>(const_pool.at(0),
                             const_pool.at(0) + const_pool.array_size()),
            (std::vector<int>{1, 5}));
