...
TEST(YggdrasilTests, CommonRawArraySetStoresFixedLengthArrays) {
  ...
  EXPECT_EQ(std::vector<int>(set.front(), set.front() + set.array_size()),
            first);
  EXPECT_EQ(std::vector<int>(set.back(), set.back() + set.array_size()),
  ...
  EXPECT_EQ(std::vector<int>(set[2], set[2] + set.array_size()),
            std::vector<int>(third.begin(), third.end()));
