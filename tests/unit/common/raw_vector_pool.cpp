...
TEST(YggdrasilTests, CommonRawVectorPoolRejectsInvalidInsertArguments) {
  ...
  const auto too_large = std::vector<int>(
...
TEST(YggdrasilTests,
     CommonRawVectorPoolMutableViewsWriteThroughToStoredValues) {
  auto pool = ygg::RawVectorPool<uint8_t, int, 32>();
  ...
  EXPECT_EQ(std::vector<int>(stored.begin(), stored.end()),
