...
void expect_shared_pool_grows_and_reuses_across_capacity_boundaries()
{
    ...
    auto pool = ygg::SharedObjectPool<SharedPoolValue, ThreadSafe>();
    auto handles = std::vector<ygg::SharedObjectPoolPtr<SharedPoolValue, ThreadSafe>> {};
    auto available = std::vector<SharedPoolValue*> {};
...
TEST(YggdrasilTests, CommonSharedObjectPoolGrowsAndReusesAcrossCapacityBoundaries)
{
    expect_shared_pool_grows_and_reuses_across_capacity_boundaries<false>();
    expect_shared_pool_grows_and_reuses_across_capacity_boundaries<true>();
}
